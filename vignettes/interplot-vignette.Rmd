...
The function visualizes the changes in the coefficient of one variable (the posterior probabilities) in a two-way interaction term conditional on the value of the other included variable. 
The plot also includes simulated 95% confidential intervals of these coefficients.In the current version, the function works with ordinary linear regression models, generalized linear models (e.g., logit, probit, ordered logit, etc.), and multilevel (mixed-effects) regressions, all with complete or multiply imputed data (in a list format).
...
`interplot` visualizes the conditional effect based on (posterior) simulated marginal effects.
...
Be aware that this method assumes that either the linear model or the one with an interaction is true. 
It is uncertain how the test would perform if neither is the true model.
An alternative way to test the significance of the conditional effect is to compare the models with and without interactions as the same through model comparison tests, such as Wald, likelihood ratio, or Hausman tests.
They may give a significant result when neither models is a true model.
These tests are also developed in a frequentist sense rather than based on the posterior simulated results.
]
...
Note that since the statistical results are presented as a `ggplot` caption, one cannot using `+ lab(caption = ...)` to specify it as for a regular `ggplot` object. 
...
ggtitle("Conditional Predicted Prob for \nLow and High Income Citizens") +
...
When a model contains a three-way interaction `var1 * var2 * var3`, the conditional effect of `var1` depends on *both* `var2` and `var3`. Supplying `var3` to `interplot` plots the effect of `var1` across `var2` at several values (or levels) of `var3`. Fixing `var3` at a value $c$ turns the three-way effect into the familiar two-way problem with a shifted intercept and slope, $(b_{1} + b_{13} c) + (b_{12} + b_{123} c)\, var2$, so the same simulation engine and statistics are reused.

For a continuous `var3`, the default conditioning values are the mean and the mean plus or minus one standard deviation (Aiken and West 1991); supply `var3_vals` to override them. A factor `var3` uses all of its levels. Three-way interactions are supported for ordinary and generalized linear models, mixed-effects models (`lmer`/`glmer`), multiply-imputed model lists, and Bayesian (`brms`) models.

```{r fig.width=7.5, fig.height=3}
m_3way <- lm(mpg ~ wt * hp * cyl, data = mtcars)

# One small-multiple panel per value of cyl (the default)
interplot(m_3way, var1 = "wt", var2 = "hp", var3 = "cyl")
```

Set `facet = FALSE` to overlay the curves in a single panel, colored by `var3`:

```{r message=FALSE}
interplot(m_3way, var1 = "wt", var2 = "hp", var3 = "cyl", facet = FALSE)
```

## Plot Nonlinear Conditional Effects

The standard multiplicative interaction model assumes that the marginal effect of `var1` is a *linear* function of the moderator `var2`. When that assumption is too restrictive, one can let `var1` interact with a transformation of `var2`---a polynomial term such as `I(var2^2)`, or a spline such as `splines::ns(var2)`. `interplot` detects these terms automatically and plots the conditional effect as the implied nonlinear function of `var2`, computed from the model's own design-matrix gradient so that the confidence band, `stats_cp`, and `adjCI` all continue to work. As with the two-way case, this is available for linear, generalized linear, mixed-effects, multiply-imputed, and Bayesian (`brms`) models.

```{r message=FALSE}
# Effect of wt allowed to vary quadratically with cyl
m_nl <- lm(mpg ~ wt * (cyl + I(cyl^2)), data = mtcars)
interplot(m_nl, var1 = "wt", var2 = "cyl")
```

A spline interaction is handled the same way; `interplot` rebuilds the spline basis at its original knots, so the conditional-effect curve can take any smooth shape the moderator implies:

```{r message=FALSE}
library(splines)
m_spline <- lm(mpg ~ wt * ns(hp, 3), data = mtcars)
interplot(m_spline, var1 = "wt", var2 = "hp")
```

To check whether the simpler linear-interaction assumption is adequate, `bin_layer()` overlays the binning estimator of Hainmueller, Mummolo, and Xu (2019): the conditional effect of `var1` estimated separately within terciles of `var2`, drawn as dot-and-whiskers at the bin medians. If those binned estimates depart from the `interplot` line, a nonlinear specification is warranted.

```{r message=FALSE}
m_lin <- lm(mpg ~ wt * cyl, data = mtcars)
interplot(m_lin, var1 = "wt", var2 = "cyl") +
  bin_layer(m_lin, var1 = "wt", var2 = "cyl")
```

## Identify Significance Regions with Johnson-Neyman Intervals

A conditional effect is often statistically significant over only part of the moderator's range. The Johnson-Neyman (JN) technique locates the exact values of `var2` at which the effect of `var1` crosses the boundary of significance. `jn_interval()` computes those bounds analytically from the model's coefficients and variance-covariance matrix (it works with `lm`, `glm`, `lmer`, and `glmer` objects):

```{r}
m_cyl <- lm(mpg ~ wt * cyl, data = mtcars)
jn <- jn_interval(m_cyl, var1 = "cyl", var2 = "wt")
print(jn)
```

`jn_layer()` turns the result into composable `ggplot2` layers---vertical lines at the JN bounds and shading over the non-significant region---so it can be added to any `interplot` output with `+`:

```{r message=FALSE}
interplot(m_cyl, var1 = "cyl", var2 = "wt") +
  jn_layer(jn)
```

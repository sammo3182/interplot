...
A user can overwrite this automatic function by setting the option `steps`, if she want to estimate over 100 points to plot or to show a phrase effect rather than a full scale in a continuously conditional variable. For example, if one only wants to show interactive effect of weight on cylinder number's influence in a quantile scale, she can set the `steps` to 5:

...
$$ As [@BramborClarkGolder2006] indicates, the above equation suggests
that its perfectly possible for the contribution of X on Y to be
...
affect the coefficient for number of cylinders on the
mileage, `var1` is `cyl`, and `var2` is `wt`.
...
when `wt` is `var1` and `cyl` is `var2`. This is because `cyl` is not a
continuous variable but a categorical one with just three values: 4, 6,
and 8. `interplot` automatically detects the number of values taken on
by `var2` and chooses the appropriate plot format. If there are fewer
than 10 values, the function will produce a "dot-and-whisker" plot;
otherwise, by default, it will generate a "line-and-ribbon" plot. Users
may override this default by setting the argument `point` to `TRUE`.
...
distribution of the conditioning variable. For this
purpose, `interplot` has the `hist` argument for users to choose to
...
cannot using `+ lab(caption = ...)` to specify it as for a
regular `ggplot` object. Instead, users can modify the plot's caption by
specify the argument `txt_caption` of the `dwplot` function.
...
[^1]: For the purpose of illustration, we omitted the county level
    variance in this example. One can see more comprehensive analyses
    in Solt et al. (2017).

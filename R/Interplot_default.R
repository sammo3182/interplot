...
interplot.default <-
  function(m,
    ...
    if (is.factor(eval(parse(text = paste0("m$model$", var1)))) &
        is.factor(eval(parse(text = paste0("m$model$", var2)))))
    ...
    if (is.factor(eval(parse(text = paste0("m$model$", var1))))) {
      var1_bk <- var1
      var1 <-
        paste0(var1, eval(parse(text = paste0(
          "m$xlevel$", var1
        ))))
      factor_v1 <- TRUE
      ifelse(
      ...
      var2_bk <- var2
      var2 <-
        paste0(var2, eval(parse(text = paste0(
          "m$xlevel$", var2
        ))))
      factor_v2 <- TRUE
      ifelse(
    ...
    if (factor_v1) {
      if (predPro == TRUE)
        stop(
          "The current version does not support estimating predicted probabilities for factor base terms."
        )
      
      for (j in 1:(length(eval(parse(
        text = paste0("m$xlevel$", var1_bk)
      ))) -
      1)) {
        ...
        for (i in 1:steps) {
          coef$coef1[i] <- mean(m.sims@coef[, match(var1[j + 1],
                                                    names(m$coef))] + coef$fake[i] * m.sims@coef[, match(var12[j],
                                                                                                         names(m$coef))])
          coef$ub[i] <-
            quantile(m.sims@coef[, match(var1[j + 1],
                                         names(m$coef))] + coef$fake[i] * m.sims@coef[, match(var12[j],
        ...
        if (adjCI == TRUE) {
          ## FDR correction
          coef$sd <-
            (coef$ub - coef$coef1) / qnorm(1 - (1 - ci) / 2)
          tAdj <-
            fdrInteraction(coef$coef1,
                           coef$sd,
                           df = m$df,
                           level = .95) # calculate critical t
      ...
      if (plot == TRUE) {
        ...
        # return coef not coef_df, since the categorical part doesn't need to be shown
      ...
      1)) {
        ...
        for (i in 1:steps) {
          ...
          coef$ub[i] <-
            quantile(m.sims@coef[, match(var1, names(m$coef))] +
                       coef$fake[i] * m.sims@coef[, match(var12[j], names(m$coef))],
                     1 - (1 - ci) / 2)
          coef$lb[i] <-
            quantile(m.sims@coef[, match(var1, names(m$coef))] +
      ...
      if (plot == TRUE) {
        if (is.null(facet_labs))
          facet_labs <- unique(coef_df$value)
        coef_df$value <-
          factor(coef_df$value, labels = facet_labs)
        interplot.plot(
          m = coef_df,
          hist = hist,
          steps = steps,
      ...
      min_sim <-
        m.sims@coef[, match(var1, names(m$coef))] + multiplier * xmin * m.sims@coef[, match(var12, names(m$coef))] # simulation of the value at the minimum value of the conditioning variable
      max_sim <-
        m.sims@coef[, match(var1, names(m$coef))] + multiplier * xmax * m.sims@coef[, match(var12, names(m$coef))] # simulation of the value at the maximum value of the conditioning variable

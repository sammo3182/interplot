...
interplot.plot <- function(m,
  ...
  if (hist == FALSE) {
    ...
    if (point == TRUE) {
      if (is.na(ercolor))
        ercolor <- "black"  # ensure whisker can be drawn
      ...
      hist.out <-
        hist(var2_dt,
             breaks = seq(min(var2_dt), max(var2_dt), l = break_var2),
             plot = FALSE)
      ...
      if (steps < 10) {
        histX <- data.frame(
          ...
          ymax = hist.out$counts / hist.max * maxdiff /
            5 + min(yrange) - maxdiff / 5,
      ...
      if (steps < 10) {
        ...
        histX_sub <-
          mutate(histX, xdiff = xmax - xmin, xmax = xmax - xdiff / 2)
      }
